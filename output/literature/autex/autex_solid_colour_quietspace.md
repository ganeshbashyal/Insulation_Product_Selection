---
title: "Autex Solid Colour Quietspace - Panel Insulation | Autex"
description: "Autex Solid Colour Quietspace is a high-performance broadband acoustic panel engineered to deliver maximum acoustic absorption finished in custom soli"
keywords: "Autex Solid Colour Quietspace, Autex panel, wall insulation, ceiling insulation, insulation Australia, Autex Australia"
status: "Draft - pending manufacturer TDS/SDS confirmation"
family_id: AUTEX_SOLID_COLOUR_QUIETSPACE
---

# Autex Solid Colour Quietspace

**Autex Panel** — acoustic panels for sound absorption and interior finish.

Autex Solid Colour Quietspace is a high-performance broadband acoustic panel engineered to deliver maximum acoustic absorption finished in custom solid colours. Built on a dual-density 100% polyester Quietspace Panel core, solid colour finishes—including custom paint matching, CMYK, and Pantone specifications—are digitally applied with safe, water-based UV-cured ink or laminated with solid-colour Vertiface fabric. It absorbs between 85% and 100% of sound energy (NRC 0.85 - 1.00) while seamlessly harmonising with any interior colour scheme.

## Key features

- Limitless solid colour finishes with custom paint, CMYK, and Pantone matching.
- Available across 25mm (NRC 0.85), 50mm (NRC 1.00), 75mm, and 100mm thicknesses.
- High-performance broadband sound absorption with dual-density polyester core.
- Printed surface or fabric overlay has zero negative impact on acoustic performance.
- Group 1 fire rating compliant with AS ISO 9705 - 2003 and AS 5637.1.
- Constructed from 100% polyester fibre containing minimum 45% to 58% recycled PET.
- Non-toxic, non-allergenic, non-irritant, with low-VOC emissions.
- Certified Carbon Neutral product with third-party verified EPD.
- Supplied with factory-applied protective spun-bond film to protect face during install.
- Backed by a 10-year manufacturer's guarantee.

## Applications and selection

- wall
- internal wall
- ceiling

**Selection checklist**

1. Specify solid colour specification (Pantone, CMYK, paint code, or Vertiface colour).
2. Select thickness (25mm for NRC 0.85 or 50mm+ for NRC 1.00).
3. Confirm substrate compatibility for adhesive direct-fix mounting.
4. Calculate total panel count based on 1200mm x 2400mm dimensions.
5. Determine need for aluminium edge trims (Autex Type C profile for 25mm).


## Current catalogue range

| Variant | Size / rating | Pack |
| --- | --- | --- |
| Solid Colour Quietspace Panel 25mm | 1200 mm x 2400 mm x 25 mm, NRC 0.85 | Panel |
| Solid Colour Quietspace Panel 50mm | 1200 mm x 2400 mm x 50 mm, NRC 1.00 | Panel |
| Solid Colour Quietspace Panel 75mm | 1200 mm x 2400 mm x 75 mm, NRC 1.00 | Panel |
| Solid Colour Quietspace Panel 100mm | 1200 mm x 2400 mm x 100 mm, NRC 1.00 | Panel |

_Variants from the manufacturer datasheet._

## Technical data

| Property | Value | Standard |
| --- | --- | --- |
| Noise Reduction Coefficient (NRC) | 25mm: 0.85 (Class C); 50mm: 1.00 (Class A); 75mm: 1.00 (Class A); 100mm: 1.00 (Class A) | ISO 354 |
| Panel Dimensions | 1200 mm x 2400 mm | - |
| Thickness Options | 25 mm, 50 mm, 75 mm, 100 mm (tolerance +/- 6%) | - |
| Weight | 25mm: 2300 gsm; 50mm: 3800 gsm; 75mm: 4050 gsm; 100mm: 4300 gsm | - |
| Material Composition | 100% polyester fibre (PET) | - |
| Recycled Content | Minimum 45% to 58% post-consumer recycled PET | - |
| Thermal Resistance | 25mm: R0.6 (@ 15°C); 50mm: R1.4 (@ 15°C) | - |
| Fire Rating (Australia) | Group 1, SMOGRArc < 100 m2/s2 | AS ISO 9705 - 2003 / AS 5637.1 |
| International Fire Rating | Group 1-S (ISO 9705:1993); Class A (ASTM E-84); 25mm BS EN 13501-1:2018 B - s2, d0; 50mm B - s2, d2 | ISO 9705 / ASTM E-84 / BS EN 13501-1 |
| Microbial Resistance | Growth rating: 0 (No growth) | ASTM G21-15 |
| VOC Emissions | < 0.092 mg/m3 (7 days) | CDPH Standard Method v1.2 |

Extracted from manufacturer datasheet: https://www.autexglobal.com/au/products/quietspace-panel/

## Fire, testing and compliance context

AS ISO 9705 - 2003 and AS 5637.1 Classification: Group 1 (SMOGRArc < 100 m2/s2); ISO 9705:1993 Group 1-S; ASTM E-84 Class A; BS EN 13501-1:2018 B - s2, d0 (25mm) / B - s2, d2 (50mm)

NCC Specification 7 (BCA C1.10) Group 1 fire compliance; AS ISO 9705:2003; AS 5637.1:2015; ISO 354

- NCC / project compliance: conditional — project-specific evidence required.
- BAL: not verified.
- Datasheet: https://www.autexacoustics.com.au/ (link audited 2026-09-05; exact product TDS may still be pending).
- SDS: https://insulvac.com.au/wp-content/uploads/2020/04/autex-material-safety-data-sheet-msds.pdf.

## Recommended accessories

- Autex Edging Solutions (Type C profile for 25mm panels).
- Contact or MS polymer adhesive.
- Pressure roller.


## Limitations and warnings

- Indoor use only; avoid direct water exposure or extreme humidity.
- Do not expose to naked flames or concentrated heat sources above 80°C.
- Printed solid colours may show slight surface marking if subjected to heavy impact.

## Installation overview

1. Inspect wall or ceiling substrate to ensure it is flat, clean, dry, and dust-free.
2. Acclimatise panels in the installation environment for at least 24 hours prior to fitting.
3. Keep protective spun-bond cover on panel face during handling and installation.
4. Apply recommended contact or high-grab MS polymer adhesive evenly across the panel back.
5. Position panel onto substrate, butt joins tightly, and roll firmly with a pressure roller.
6. Peel back protective film corners during painting and remove fully upon completion.

## Safety and handling

Confirm the current SDS before handling or cutting. No product-specific hazard classification is asserted here.

## Sustainability and indoor environment

Carbon neutral product; minimum 45% to 58% recycled PET; Declare Red List Free; EPD compliant; low VOC

## Warranty, returns and support

Manufacturer's Guarantee (10 years)

## Specification starting point

> Insulation shall be Autex Solid Colour Quietspace (panel) by Autex, selected to suit the confirmed application and required rating. Exact product, thickness and compliance to be confirmed by the project reviewer against the current manufacturer TDS before ordering.

## Source register

- SRC-01 Manufacturer datasheet: https://www.autexacoustics.com.au/.
- SRC-02 Safety data sheet: https://insulvac.com.au/wp-content/uploads/2020/04/autex-material-safety-data-sheet-msds.pdf.
- SRC-03 Internal SKU catalogue (3 variant(s)).

## Review actions before publication

- Confirm the exact product TDS deep link and re-validate all technical values against it.
- Confirm SDS currency and handling guidance.
- Approve environmental and warranty wording with the manufacturer.
- Human review of NCC, fire, BAL and acoustic claims remains mandatory.

_Draft generated 2026-09-05 from the internal knowledge base. Technical values and claims remain subject to manufacturer review before publication._
