---
title: "Autex Vicinity - Desk Screen - Nook - Batt Insulation | Autex"
description: "Autex Vicinity Desk Screen - Nook is a comprehensive three-sided acoustic workstation enclosure designed to create an intimate, distraction-free booth"
keywords: "Autex Vicinity - Desk Screen - Nook, Autex batt, wall insulation, insulation Australia, Autex Australia"
status: "Draft - pending manufacturer TDS/SDS confirmation"
family_id: AUTEX_VICINITY_DESK_SCREEN_NOOK
---

# Autex Vicinity - Desk Screen - Nook

**Autex Batt** — bulk insulation batts for thermal and acoustic performance.

Autex Vicinity Desk Screen - Nook is a comprehensive three-sided acoustic workstation enclosure designed to create an intimate, distraction-free booth around individual desks. Wrapping the back and both sides of a workstation, Nook delivers an outstanding sound absorption rating of 1.55 Sabins per unit to shield workers from peripheral noise and visual motion. Fixed non-destructively with Vicinity Clamps, it is formed from 100% polyester fibre with an 80% recycled PET core and meets Group 1 fire safety standards.

## Key features

- Three-sided wrap-around pod enclosure creating maximum physical and acoustic privacy.
- Delivers high acoustic sound absorption of 1.55 Sabins per unit.
- Non-destructive desk mounting using Vicinity Workstation Clamps.
- Constructed from 100% polyester fibre containing a minimum of 80% recycled PET.
- Group 1 fire rating compliant with AS ISO 9705 - 2003 and AS 5637.1.
- Solid colour throughout with clean finished edges; no capping required.
- Pinnable self-healing surface on all three internal faces.
- Clamps made from 100% recycled PET (RePET) or powdercoated zinc.
- Moisture and mould resistant with an ASTM G21-15 Growth Rating of 0.
- Certified Carbon Neutral product with third-party verified EPD.

## Applications and selection

- internal wall

**Selection checklist**

1. Select size matching desk width (Nook 1600 for 1600mm desks; Nook 1800 for 1800mm desks).
2. Confirm desktop depth and edge overhang allow clamp installation on three sides.
3. Select colour from the Autex Cube palette.
4. Confirm pack quantity (supplied in packs of 4 complete screens).
5. Verify that side return wings do not impede user movement or drawer pedestals.


## Current catalogue range

| Rating | Type | Thickness | Dimensions | SKUs |
| --- | --- | --- | --- | --- |
| Rw 35 | acoustic_rw | Varies | 1160 (width varies) | 2 |

## Technical data

| Property | Value | Standard |
| --- | --- | --- |
| Acoustic Performance | 1.55 Sabins per unit (125Hz: 0.50, 250Hz: 1.00, 500Hz: 1.40, 1000Hz: 1.65, 2000Hz: 2.10, 4000Hz: 2.35) | ISO 354 |
| Dimensions | 2400 mm W x 600 mm H flat (3 pieces per screen, wrapped around 1600 mm or 1800 mm desks, ~400 mm above desk) | - |
| Thickness | 12 mm (+/- 6%) | - |
| Material Composition | 100% polyester fibre (PET) Cube panel; clamp: 100% recycled PET / zinc | - |
| Recycled Content | Minimum 80% post-consumer recycled PET | - |
| Fire Rating (Australia) | Group 1, SMOGRArc < 100 m2/s2 | AS ISO 9705 - 2003 / AS 5637.1:2015 |
| International Fire Classification | Group 1-S (ISO 9705:1993); Class A (ASTM E-84-15a: FS:0 - SD:45); BS EN 13501-1:2018 | ISO 9705 / ASTM E-84 / BS EN 13501-1 |
| Water Vapour Sorption | 0.4% by weight after 4 days at 49°C, 95% RH | ASTM C1104 / C1104M-13a |
| Microbial Resistance | Growth rating: 0 (No growth) | ASTM G21-15 |
| VOC Emissions | < 0.092 mg/m3 (7 days) | CDPH Standard Method v1.2 |

Extracted from manufacturer datasheet: https://www.autexglobal.com/au/products/vicinity/

## Fire, testing and compliance context

AS ISO 9705 - 2003 and AS 5637.1 Classification: Group 1 (SMOGRArc < 100 m2/s2); ISO 9705:1993 Group 1-S; ASTM E-84-15a Class A; BS EN 13501-1:2018

NCC Specification 7 (BCA C1.10) Group 1 fire compliance; AS ISO 9705:2003; AS 5637.1:2015; ISO 354

- NCC / project compliance: conditional — project-specific evidence required.
- BAL: not verified.
- Datasheet: https://www.autexacoustics.com.au/ (link audited 2026-09-05; exact product TDS may still be pending).
- SDS: https://insulvac.com.au/wp-content/uploads/2020/04/autex-material-safety-data-sheet-msds.pdf.

## Recommended accessories

- Vicinity Workstation Clamps.
- Allen key.


## Limitations and warnings

- Indoor use only; avoid wet areas or direct liquid contact.
- Workstation desk must accommodate side and rear clamps.
- Sold in pack quantities of 4 complete screens.

## Installation overview

1. Unpack the 3-piece Nook screen set and verify all components.
2. Attach Vicinity Desk Clamps to pre-routed sockets on rear and side panels.
3. Align the three panels around the back and both sides of the desktop.
4. Tighten the clamp mounting bolts from underneath the desk using an Allen key.
5. Ensure all corners fit neatly and the enclosure stands rigidly around the workspace.

## Safety and handling

Confirm the current SDS before handling or cutting. No product-specific hazard classification is asserted here.

## Sustainability and indoor environment

Carbon neutral product; minimum 80% recycled PET; clamps made from 100% recycled PET; Declare Red List Free; EPD verified; low VOC

## Warranty, returns and support

Manufacturer's Guarantee (10 years)

## Specification starting point

> Insulation shall be Autex Vicinity - Desk Screen - Nook (batt) by Autex, selected to suit the confirmed application and required rating. Exact product, thickness and compliance to be confirmed by the project reviewer against the current manufacturer TDS before ordering.

## Source register

- SRC-01 Manufacturer datasheet: https://www.autexacoustics.com.au/.
- SRC-02 Safety data sheet: https://insulvac.com.au/wp-content/uploads/2020/04/autex-material-safety-data-sheet-msds.pdf.
- SRC-03 Internal SKU catalogue (1 variant(s)).

## Review actions before publication

- Confirm the exact product TDS deep link and re-validate all technical values against it.
- Confirm SDS currency and handling guidance.
- Approve environmental and warranty wording with the manufacturer.
- Human review of NCC, fire, BAL and acoustic claims remains mandatory.

_Draft generated 2026-09-05 from the internal knowledge base. Technical values and claims remain subject to manufacturer review before publication._
