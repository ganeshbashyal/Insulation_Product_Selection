---
title: "Autex Surface Finishes Frontier Raft beam - Batt Insulation | Autex"
description: "Autex Surface Finishes Frontier Raft - Beam (including Acoustic Timber Raft Beam) is an innovative acoustic ceiling beam system that combines the warm"
keywords: "Autex Surface Finishes Frontier Raft beam, Autex batt, ceiling insulation, insulation Australia, Autex Australia"
status: "Draft - pending manufacturer TDS/SDS confirmation"
family_id: AUTEX_SURFACE_FINISHES_FRONTIER_RAFT_BEAM
---

# Autex Surface Finishes Frontier Raft beam

**Autex Batt** — bulk insulation batts for thermal and acoustic performance.

Autex Surface Finishes Frontier Raft - Beam (including Acoustic Timber Raft Beam) is an innovative acoustic ceiling beam system that combines the warmth and appearance of real timber or architectural stone with high-performance sound absorption. Built from scored and folded 12mm Cube polyester felt digitally printed with high-resolution woodgrains or stone textures using water-based UV-cured ink, it is available in Beam 100 (87mm high) and Beam 250 (227mm high) profiles. It delivers an NRC of 0.75 to 0.95 and Group 1 fire safety without the weight, cost, or warping of solid timber beams.

## Key features

- Realistic digitally printed timber woodgrains (Acoustic Timber) and stone textures.
- Available in Beam 100 (87mm depth) and Beam 250 (227mm depth) profiles.
- Internal sound-dampening air gap enhances acoustic performance (NRC 0.75 - 0.95).
- Printed with safe, water-based, UV-cured ink that does not compromise acoustics.
- Lightweight, seismic-safe alternative to solid wood or steel ceiling beams.
- Made from 100% polyester fibre containing a minimum of 60% to 80% recycled PET.
- Group 1 fire rating compliant with AS ISO 9705 - 2003 and AS 5637.1.
- Compatible with Frontier mounting clips and adjustable suspension systems.
- Will not warp, crack, splinter, or slump over time.
- Certified Carbon Neutral product with third-party verified EPD.

## Applications and selection

- ceiling
- open-plan office
- boardroom
- commercial interior
- education
- hospitality

**Selection checklist**

1. Select woodgrain (Acoustic Timber) or stone finish from Surface Finishes palette.
2. Select Beam depth (Beam 100 at 87mm or Beam 250 at 227mm).
3. Confirm beam length (1200mm or 2400mm; custom lengths available).
4. Verify beam spacing (150mm for Beam 100; 300mm for Beam 250) to calculate pack requirements.
5. Determine direct-fix or suspended installation hardware requirement.


## Current catalogue range

| Rating | Type | Thickness | Dimensions | SKUs |
| --- | --- | --- | --- | --- |
| Rw 50 | acoustic_rw | 2400 | 2400 (width varies) | 2 |
| Rw 50 | acoustic_rw | 2700 | 2700 (width varies) | 2 |

## Technical data

| Property | Value | Standard |
| --- | --- | --- |
| Acoustic Performance | NRC 0.75 - 0.95 (Beam 100: NRC 0.80 at 150mm centres, 200mm off ceiling; Beam 250: NRC 0.90 at 300mm centres) | ISO 354 |
| Profile Dimensions (Beam 100) | 70 mm width x 87 mm height (2400 mm or 1200 mm length) | - |
| Profile Dimensions (Beam 250) | 70 mm width x 227 mm height (2400 mm or 1200 mm length) | - |
| Base Material Thickness | 12 mm Cube panel | - |
| Material Composition | 100% polyester fibre (PET) printed with UV-curable ink, aluminium extrusion | - |
| Recycled Content | Minimum 60% to 80% post-consumer recycled PET | - |
| Fire Hazard Classification | Group 1, SMOGRArc < 100 m2/s2 | AS ISO 9705 - 2003 / AS 5637.1:2015 |
| International Fire Rating | Group 1-S (ISO 9705:1993); Class A (ASTM E-84-15a: FS:0 - SD:45); B - s2, d0 (BS EN 13501-1:2018) | ISO 9705 / ASTM E-84 / BS EN 13501-1 |
| Water Vapour Sorption | 0.4% by weight after 4 days at 49°C, 95% RH | ASTM C1104 / C1104M-13a |
| Microbial Resistance | Growth rating: 0 (No growth) | ASTM G21-15 |
| VOC Emissions | < 0.092 mg/m3 (7 days) | CDPH Standard Method v1.2 |

Extracted from manufacturer datasheet: https://www.autexglobal.com/au/products/frontier/

## Fire, testing and compliance context

AS ISO 9705 - 2003 and AS 5637.1 Classification: Group 1 (SMOGRArc < 100 m2/s2); ISO 9705:1993 Group 1-S; ASTM E-84-15a Class A; BS EN 13501-1:2018 B - s2, d0

NCC Specification 7 (BCA C1.10) Group 1 fire compliance; AS ISO 9705:2003; AS 5637.1:2015; ISO 354

- NCC / project compliance: conditional — project-specific evidence required.
- BAL: not verified.
- Datasheet: https://www.autexacoustics.com.au/ (link audited 2026-09-05; exact product TDS may still be pending).
- SDS: https://insulvac.com.au/wp-content/uploads/2020/04/autex-material-safety-data-sheet-msds.pdf.

## Recommended accessories

- Frontier Raft Extrusions (RAFNEXRT).
- Autex Mounting Clips (RAFTCC).
- Frontier Channel Connectors (RAFCCT).
- Adjustable suspension cable sets / rod hangers.
- Frontier Beam end caps.


## Limitations and warnings

- Indoor ceiling application only; not for external use or wet environments.
- Non-load-bearing beam; cannot support luminaires or heavy ceiling services directly.
- Handle with clean hands or white gloves during installation to prevent marking the printed surface.

## Installation overview

1. Plan ceiling layout allowing recommended spacing (150mm centres for Beam 100; 300mm centres for Beam 250).
2. Install Frontier mounting clips or suspension track securely to structural ceiling.
3. Slide RAFNEXRT aluminium extrusion into the folded Beam raft top channel.
4. Align Autex mounting clips with channel and turn 45 degrees to temporarily position.
5. Adjust along length, check grain alignment, and twist clips 90 degrees to lock firmly in place.
6. Fit black extrusion end caps to beam ends for a clean architectural finish.

## Safety and handling

Confirm the current SDS before handling or cutting. No product-specific hazard classification is asserted here.

## Sustainability and indoor environment

Contains minimum 60% to 80% recycled PET; water-based UV-cured ink; zero carbon manufacturing; Declare Red List Free; EPD verified; low VOC

## Warranty, returns and support

10 Year Manufacturer's Guarantee

## Specification starting point

> Insulation shall be Autex Surface Finishes Frontier Raft beam (batt) by Autex, selected to suit the confirmed application and required rating. Exact product, thickness and compliance to be confirmed by the project reviewer against the current manufacturer TDS before ordering.

## Source register

- SRC-01 Manufacturer datasheet: https://www.autexacoustics.com.au/.
- SRC-02 Safety data sheet: https://insulvac.com.au/wp-content/uploads/2020/04/autex-material-safety-data-sheet-msds.pdf.
- SRC-03 Internal SKU catalogue (2 variant(s)).

## Review actions before publication

- Confirm the exact product TDS deep link and re-validate all technical values against it.
- Confirm SDS currency and handling guidance.
- Approve environmental and warranty wording with the manufacturer.
- Human review of NCC, fire, BAL and acoustic claims remains mandatory.

_Draft generated 2026-09-05 from the internal knowledge base. Technical values and claims remain subject to manufacturer review before publication._
