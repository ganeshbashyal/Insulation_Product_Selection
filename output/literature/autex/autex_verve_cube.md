---
title: "Autex Verve Cube - Panel Insulation | Autex"
description: "Autex Verve Cube is a sculpted 3D acoustic wall panel that animates spaces through the interplay of light, shadow, and architectural form. Formed from"
keywords: "Autex Verve Cube, Autex panel, wall insulation, insulation Australia, Autex Australia"
status: "Draft - pending manufacturer TDS/SDS confirmation"
family_id: AUTEX_VERVE_CUBE
---

# Autex Verve Cube

**Autex Panel** — acoustic panels for sound absorption and interior finish.

Autex Verve Cube is a sculpted 3D acoustic wall panel that animates spaces through the interplay of light, shadow, and architectural form. Formed from 100% polyester fibre with an 80% recycled PET core, Verve Cube features solid colour throughout without requiring edge capping. Available in six distinctive profiles—Pillar, Flute, Ovolo, Pinstripe, Ridge, and Windswept—across the full Autex Cube colour palette, it delivers sound absorption (NRC 0.30+) and Group 1 fire compliance.

## Key features

- Six sculptural contoured profiles: Pillar, Flute, Ovolo, Pinstripe, Ridge, Windswept.
- Solid through-core colour across the entire Autex Cube 23+ colour palette.
- Constructed from 100% polyester fibre containing a minimum of 80% recycled PET.
- Available in two thicknesses: 12mm (6mm edge) and 21mm (15mm edge).
- Full-height wall panel format measuring 2680mm or 2700mm height by 1170mm width.
- Acoustic rating of NRC 0.30 and higher.
- Group 1 fire rating compliant with AS ISO 9705 - 2003 and AS 5637.1.
- Seamless joint design where panels join at pattern low points.
- Glue-free SpinFix mechanical mounting or direct adhesive fixing options.
- Certified Carbon Neutral product with third-party verified EPD.

## Applications and selection

- wall
- internal wall

**Selection checklist**

1. Select contoured style (Pillar, Flute, Ovolo, Pinstripe, Ridge, Windswept).
2. Select colour from the Autex Cube 23+ solid colour palette.
3. Select thickness (12mm with 6mm edge or 21mm with 15mm edge).
4. Confirm wall height to choose 2680mm or 2700mm lengths.
5. Choose mounting method (adhesive direct-fix or SpinFix mechanical clips).


## Current catalogue range

| Rating | Type | Thickness | Dimensions | SKUs |
| --- | --- | --- | --- | --- |
| Rw 35 | acoustic_rw | 12 | 2700 x 1170 | 2 |
| Rw 35 | acoustic_rw | 21 | 2700 x 1170 | 2 |

## Technical data

| Property | Value | Standard |
| --- | --- | --- |
| Acoustic Rating (NRC) | NRC 0.30 and higher (typically 0.30 - 0.70+ depending on air gap) | ISO 354 |
| Panel Dimensions | 1170 mm width x 2680 mm length (or 2700 mm length; custom up to 2680 mm) | - |
| Thickness Options | 12 mm (6 mm edge) | 21 mm (15 mm edge) | - |
| Base Material | 100% polyester fibre (PET) Cube panel | - |
| Recycled Content | Minimum 80% post-consumer recycled PET | - |
| Fire Rating (Australia) | Group 1, SMOGRArc < 100 m2/s2 | AS ISO 9705 - 2003 / AS 5637.1:2015 |
| International Fire Classification | Group 1-S (ISO 9705:1993); BS EN 13501-1:2018 B - s1, d0 | ISO 9705 / BS EN 13501-1 |
| Water Vapour Sorption | 0.4% by weight after 4 days at 49°C, 95% RH | ASTM C1104 / C1104M-13a |
| Microbial Resistance | Growth rating: 0 (No growth) | ASTM G21-15 |
| VOC Emissions | < 0.092 mg/m3 (7 days) | CDPH Standard Method v1.2 |

Extracted from manufacturer datasheet: https://www.autexglobal.com/au/products/verve/

## Fire, testing and compliance context

AS ISO 9705 - 2003 and AS 5637.1 Classification: Group 1 (SMOGRArc < 100 m2/s2); ISO 9705:1993 Group 1-S; BS EN 13501-1:2018 B - s1, d0

NCC Specification 7 (BCA C1.10) Group 1 fire compliance; AS ISO 9705:2003; AS 5637.1:2015; ISO 354

- NCC / project compliance: conditional — project-specific evidence required.
- BAL: not verified.
- Datasheet: https://www.autexacoustics.com.au/ (link audited 2026-09-05; exact product TDS may still be pending).
- SDS: https://insulvac.com.au/wp-content/uploads/2020/04/autex-material-safety-data-sheet-msds.pdf.

## Recommended accessories

- Autex SpinFix mounting brackets.
- High-grab MS polymer adhesive.
- Fine-tooth saw.


## Limitations and warnings

- Indoor use only; avoid wet areas or direct water exposure.
- Pinstripe profile is available in 12mm thickness only.
- Panels must be installed in uniform vertical orientation.

## Installation overview

1. Ensure wall substrate is smooth, flat, clean, and dry.
2. Acclimatise panels in the space for at least 24 hours prior to installation.
3. Inspect profile repeat and plan alignment across the wall area.
4. Apply high-grab MS polymer adhesive evenly across panel backing or attach SpinFix clips.
5. Position panel on wall, engage seams tightly at pattern low points, and press firmly across face.
6. Trim perimeter cuts cleanly using a track saw with a fine-toothed blade.

## Safety and handling

Confirm the current SDS before handling or cutting. No product-specific hazard classification is asserted here.

## Sustainability and indoor environment

Carbon neutral product; minimum 80% recycled PET; Declare Red List Free; EPD compliant; low VOC

## Warranty, returns and support

Manufacturer's Guarantee (10 years)

## Specification starting point

> Insulation shall be Autex Verve Cube (panel) by Autex, selected to suit the confirmed application and required rating. Exact product, thickness and compliance to be confirmed by the project reviewer against the current manufacturer TDS before ordering.

## Source register

- SRC-01 Manufacturer datasheet: https://www.autexacoustics.com.au/.
- SRC-02 Safety data sheet: https://insulvac.com.au/wp-content/uploads/2020/04/autex-material-safety-data-sheet-msds.pdf.
- SRC-03 Internal SKU catalogue (2 variant(s)).

## Review actions before publication

- Confirm the exact product TDS deep link and re-validate all technical values against it.
- Confirm SDS currency and handling guidance.
- Approve environmental and warranty wording with the manufacturer.
- Human review of NCC, fire, BAL and acoustic claims remains mandatory.

_Draft generated 2026-09-05 from the internal knowledge base. Technical values and claims remain subject to manufacturer review before publication._
