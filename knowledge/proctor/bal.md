...
This is the canonical internal description for the Proctor BAL family (Mesh category). It aligns the terminology used by the enquiry bot, sales team and future Aircall CSV.

For the demonstration, the bot may recommend the **Proctor BAL family** when the customer's problem matches its documented applications. It must not choose a specific grade, calculate order quantity, or confirm thermal, acoustic, fire, NCC or BAL compliance for a project. Those decisions remain human-reviewed.

## Canonical description

Quality insulation engineered for Australian conditions. Engineered to meet Australian building requirements. Manufacturer-published features and benefits include: Quality insulation product for Australian building conditions; designed to meet NCC energy efficiency requirements; fire-rated construction for FRL requirements; BCA compliant fire ratings This family covers 1 SKU variant(s) in the current catalogue. It is not a complete compliant building system on its own; project-specific claims about a finished construction must be confirmed by a human reviewer against a tested system that matches the proposed build-up.

## Current manufacturer-supported facts

- Product type: Mesh insulation product from Proctor.
- Material: Metal Mesh.
...
### Within the Proctor BAL family

- Bushfire / Ember Protection

### Separate product families

Do not transfer claims from this family to other Proctor product families without their own current technical evidence. Where Proctor sells multiple product families within the Mesh category or in other categories, each is a distinct family with its own grade table and evidence.
...
- ensure minimum 20% compression for acoustic performance.
...
- fire ratings are system-based - verify complete assembly matches tested configuration.
...
| Requirement | Status for Proctor BAL | Enquiry-bot action |
| --- | --- | --- |
...
> Proctor BAL is a manufacturer-supported product family used for thermal insulation (R-value). The right grade depends on the complete construction and project requirements. I can collect the details for our team to review.
...
1. Select ProctorVent BAL-ready
vents pre-fitted with ≤ 2mm metal mesh.
2. For eaves ventilation, fix FV10 BAL or
FV25 BAL over the top edge of the fascia board prior to installing roof sheeting
or gutters.
3. For ridge ventilation, install RV10 BAL continuously along the
roof ridge over the cut roof underlay.
4. Ensure the BAL mesh completely covers
all ventilation openings with zero gaps exceeding 2mm.
5. Secure vents
mechanically into timber or metal fascia and rafters using non-corrosive
fasteners.
6. Fix roof cladding over the assembly ensuring the ember-guarded
ventilation pathway remains open and unobstructed.

**Manufacturer-stated limitations**

- Mesh openings must be kept clear of paint, sealant, and
construction dust during building works.
- Do not install without roof underlay;
designed as part of a complete roof envelope system.
- Ensure all fascia and
ridge junctions are tightly fitted with no bypass gaps greater than 2mm.
...
| --- | --- | --- |
| Mesh Aperture Size | ≤ 2.0 mm (complies with ember
guard requirements) | AS 3959 |
...
| Vent Airflow Capacity (FV25 BAL) | 25,000 mm² per linear
metre (1/150 ratio) |  |
| Vent Airflow Capacity
(RV10 BAL) | 10,000 mm² per linear metre |  |
| Bushfire Attack Level Suitability | BAL-12.5, BAL-19,
BAL-29, BAL-40, and BAL-FZ (in compliant assemblies) | AS 3959 |
| Combustibility | Non-combustible metal mesh | AS 1530.1 |
| Service Temperature | -40°C
to +90°C (vent body), mesh exceeds 500°C |  |
