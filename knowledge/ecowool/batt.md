---
id: ecowool-batt
family_id: ECOWOOL_BATT
manufacturer: Ecowool
category: Batt
canonical_name: Ecowool Batt
material: Polyester
...
This is the canonical internal description for the Ecowool Batt family. It aligns the terminology used by the enquiry bot, sales team and future Aircall CSV.

For the demonstration, the bot may recommend the **Ecowool Batt family** when the customer's problem matches its documented applications. It must not choose a specific grade, calculate order quantity, or confirm thermal, acoustic, fire, NCC or BAL compliance for a project. Those decisions remain human-reviewed.

## Canonical description

Quality insulation engineered for Australian conditions. Safe to handle without gloves or masks. Made from recycled plastic bottles for sustainability. Manufacturer-published features and benefits include: 100% polyester - safe to handle without protective equipment; contains minimum 80% recycled PET plastic bottles; non-allergenic, non-irritant formulation; will not support mould or mildew growth; maintains performance when wet This family covers 54 SKU variant(s) in the current catalogue. It is not a complete compliant building system on its own; project-specific claims about a finished construction must be confirmed by a human reviewer against a tested system that matches the proposed build-up.
...
- The catalogue includes 54 unique SKU variant(s) in this family.
...
| Requirement | Status for Ecowool Batt | Enquiry-bot action |
| --- | --- | --- |
...
> Ecowool Batt is a manufacturer-supported product family used for both thermal insulation (R-value) and acoustic performance (Rw/NRC). The right grade depends on the complete construction and project requirements. I can collect the details for our team to review.
...
The master product catalogue provides current commercial records for 54 SKUs, including internal SKU codes, dimensions, grade labels and stock. Commercial data does not validate technical performance; grade labels must be cross-checked against current manufacturer TDS per SKU before quoting.
